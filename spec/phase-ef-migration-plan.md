...
│  - PostgreSQL: схема, индексы, скрипт первого администратора, seed-категории │
...
| **TRC-2** | Database Infrastructure | Phase D | 0 | SQL-скрипты в `src/backend/db/`, схема, индексы, скрипт первого администратора (выполнение — по необходимости, допускается до итерации 3) |
...
| Итерация | Название | Цель | Критерии готовности |
|----------|----------|------|---------------------|
| **0** | Фундамент | Инфраструктура, БД, каркас backend/frontend | Backend отвечает на health-check; Frontend отображает заглушку; БД создана |
| **1** | Аутентификация и события | Регистрация, вход, создание событий, каталог | Организатор может создать событие; посетитель видит каталог; можно зарегистрироваться на платформе |
| **2** | Регистрация и уведомления | Регистрация на мероприятия, email | Посетитель может зарегистрироваться на мероприятие; получает email; ЛК участника работает |
| **3** | Управление и администрирование | Чек-ин, экспорт, модерация | Организатор управляет участниками, экспортирует; администратор модерирует |
| **4** | Интеграция и развёртывание | Сборка, раздача статики, deployment | Система развёрнута; ручное тестирование пройдено |

### 6.2 Последовательность изменений (детально)

**Порядок выполнения:** строгая последовательность; каждый WP начинается только после завершения предыдущего в цепочке.

#### Итерация 0: Фундамент

**Последовательность:** WP-0.1 → WP-0.2 → WP-0.3 → WP-0.4 → WP-0.5 → WP-0.6 → WP-0.7 → WP-0.8
...
| WP-0.5 | Скрипт первого администратора | SQL в `src/backend/db/` для создания admin; выполнение скрипта — по необходимости, допускается отложить до итерации 3 (когда понадобится админ-панель) | Скрипт готов |
| WP-0.6 | Seed-категории | SQL-скрипт начальных категорий (Конференции, Вебинары, Концерты и т.п.) в `src/backend/db/` | Категории доступны для выбора при создании события |
| WP-0.7 | Frontend каркас | React + TypeScript + Vite, npm, конфиг API (JSON в src/) | SPA собирается |
| WP-0.8 | SMTP для dev | MailHog или Ethereal, настройка в appsettings | Email в dev работает |

#### Итерация 1: Аутентификация и события

**Последовательность:** WP-1.1 → WP-1.2 → WP-1.3 → WP-1.4 → WP-1.5 → WP-1.6
...
**Последовательность:** WP-2.1 → WP-2.2 → WP-2.3 → WP-2.4 → WP-2.5
...
**Последовательность:** WP-3.1 → WP-3.2 → WP-3.3 → WP-3.4 → WP-3.5
...
**Последовательность:** WP-4.1 → WP-4.2 → WP-4.3 → WP-4.4 → WP-4.5
...
| WP-4.4 | Ручное тестирование | Проверка по чек-листу сценариев из Phase B (FR-01–FR-09) | Дефекты исправлены |
| WP-4.5 | Документация | README, инструкция по развёртыванию | Документация готова |

### 6.3 Детальные Work Packages

Детализация WP (1–2 часа каждый) с входными артефактами, ожидаемым результатом и критериями проверки — в отдельном документе: **[phase-ef-work-packages.md](./phase-ef-work-packages.md)**.

### 6.4 Transition Architecture States
...
|--------|--------|---------|
| **Зависимости** | TRC-1 → все остальные; RC-3 → RC-4, RC-5, RC-7 | Строгая последовательность итераций 0 → 1 → 2 → 3 → 4; детальный порядок WP — [phase-ef-work-packages.md](./phase-ef-work-packages.md) |
