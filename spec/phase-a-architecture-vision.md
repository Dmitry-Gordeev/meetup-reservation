...
- **Посетитель (участник):**
  - может зарегистрироваться на платформе — тогда при регистрации на мероприятие данные (ФИО, email, телефон) подставляются автоматически; у зарегистрированного участника есть личный кабинет («Мои регистрации»);
  - может участвовать без регистрации на платформе — вводит данные при каждой регистрации на мероприятие (гостевая регистрация).
- **Администратор платформы:** отдельная роль с правами модерации и управления пользователями; авторизация обязательна. Первый администратор создаётся через отдельный скрипт или консольную команду.
...
| **Администратор платформы** | Оператор системы | Модерация событий и организаторов (постмодерация, блокировка); управление пользователями (просмотр, блокировка); управление категориями |
...
- **Функциональность:**
  ...
  - Каталог событий: единый каталог (поиск и фильтрация по публичным событиям, фиксированный список категорий) и страницы организаторов (события конкретного организатора; публичный профиль организатора: название, описание, аватар). Неавторизованный посетитель видит публичный каталог, может перейти к регистрации на мероприятие, войти в систему или зарегистрироваться на платформе. Приватные события — доступ по ссылке.
  - Регистрация и оплата посетителей.
  - Управление участниками, чек-ин, отмена/возврат.
  - Администрирование: модерация событий и организаторов (одобрение, блокировка); управление пользователями (просмотр, блокировка).
...
- Схема БД управляется SQL-скриптами, миграции — ручные или через скрипты. Создание первого администратора — через отдельный скрипт/консольную команду.
...
- Добавление названия, описания, даты/времени, места (онлайн/офлайн; для онлайн — ссылка на трансляцию при необходимости указывается в описании), категории из фиксированного списка платформы (конференции, вебинары, концерты и т.п.).
...
- Выбор типа билета; ввод данных (ФИО, email, телефон) — для гостей или при первом участии; автозаполнение — для зарегистрированных пользователей. Один участник — один билет на событие (несколько билетов на одно событие — не в первой версии). Запрет дублирования: один email — один билет на событие.
...
- Экспорт данных в Excel/PDF: список участников события (ФИО, email, телефон, статус чек-ина и т.п.).
