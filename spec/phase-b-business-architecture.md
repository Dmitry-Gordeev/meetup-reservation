...
| BC-02 | **Управление категориями** | Создание, редактирование, пометка архивной (в новых событиях недоступна) | Администратор |
| BC-03 | **Управление событиями** | Создание, публикация, отмена событий; типы билетов; публичность | Организатор |
| BC-04 | **Управление каталогом** | Единый каталог, страницы организаторов, фильтрация по категориям, сортировка (по дате проведения, по дате создания) | Платформа |
...
|------|----------|----------|-----------|
| 1 | Просмотр каталога / фильтрация по категориям | Посетитель | Найденное событие |
...
2. Создаёт событие: название, описание, дата/время, место (онлайн/офлайн), категории (обязательно, можно выбрать несколько), публичность.
...
**Ограничения:** минимум один тип билета обязателен; редактирование после публикации не предусмотрено; клонирование — не в первой версии.

#### BP-02: Регистрация на мероприятие (Посетитель)

1. Посетитель находит событие (каталог с фильтрацией по категориям или страница организатора).
2. Выбирает тип билета.
3. Вводит данные (Фамилия, Имя, Отчество, email — обязательны; телефон — опционально) или использует автозаполнение (если зарегистрирован).
...
2. При необходимости блокирует или разблокирует событие (скрыто, доступно по ссылке), организатора, участника.
3. При блокировке организатора все его события отменяются, участники получают уведомление. При разблокировке отменённые события не восстанавливаются.
4. Управляет пользователями (просмотр, блокировка). При блокировке участника: вход запрещён, все его регистрации аннулируются, организаторы получают уведомление.
5. Добавляет и редактирует категории; может пометить категорию архивной (в старых событиях остаётся, в новых её добавить нельзя).
...
│   ├── Сортировка (по дате проведения, по дате создания)
...
│
├── Уведомления
│   ├── Подтверждение регистрации (участнику)
│   ├── Напоминания (день/час)
│   ├── Уведомление об отмене
│   └── Уведомление организатору о новой регистрации (не в первой версии)
│
...
│
└── Администрирование
    ├── Модерация событий (блокировка/разблокировка)
    ├── Блокировка/разблокировка организаторов и участников
    ├── Управление пользователями
    └── Управление категориями (добавление, редактирование, пометка архивной)
```
...
| G5 | Нет каталога | Единый каталог с фильтрацией и сортировкой | Новая система | Высокий |
...
| ID | Требование | Приоритет |
|----|------------|-----------|
| FR-02.1 | Создание события: название, описание, дата/время, место, категории (обязательно, можно выбрать несколько) | Must |
| FR-02.2 | Публичность: публичное / приватное (по ссылке) | Must |
| FR-02.3 | Несколько типов билетов (название, цена, вместимость); минимум один тип обязателен | Must |
...
| FR-03.2 | Фильтрация по категориям: логика OR (событие показывается, если имеет хотя бы одну из выбранных категорий); архивные участвуют; полнотекстовый поиск — не в первой версии | Must |
| FR-03.2a | Сортировка каталога: по дате проведения, по дате создания (выбор пользователем) | Must |
...
| FR-03.7 | Полнотекстовый поиск по названию/описанию | Won't (след. фаза) |
...
| FR-04.2 | Ввод данных: Фамилия, Имя, Отчество (три поля) и email — обязательны; телефон — опционально | Must |
...
| FR-05.4 | Экспорт в Excel/PDF (Фамилия, Имя, Отчество, email, телефон при наличии, статус чек-ина) | Must |
...
| FR-07.5 | Email: уведомление организатору при блокировке участника (регистрации аннулируются) | Must |
| FR-07.6 | Email: уведомление организатору о новой регистрации | Won't (не в первой версии) |
| FR-07.7 | SMS | Won't (след. фаза) |

#### FR-08: Администрирование

| ID | Требование | Приоритет |
|----|------------|-----------|
| FR-08.1 | Постмодерация: блокировка и разблокировка события | Must |
| FR-08.2 | Блокировка и разблокировка организатора. При блокировке: все события отменяются, уведомления. При разблокировке: отменённые события не восстанавливаются, организатор создаёт новые | Must |
| FR-08.3 | Управление пользователями: просмотр, блокировка, разблокировка. При блокировке участника: вход запрещён, все его регистрации аннулируются, организаторы получают уведомление | Must |
| FR-08.4 | Управление категориями: добавление, редактирование, пометка архивной. Архивная категория остаётся в старых событиях, в новых её добавить нельзя | Must |
...
| FR-09.2 | Лимиты по каждому типу билета; минимум один тип билета на событие | Must |
...
*Документ подготовлен в формате TOGAF Phase B (Business Architecture) на основе [Phase A: Architecture Vision](./phase-a-architecture-vision.md). Следующие этапы: [Phase C: Information Systems Architectures](./phase-c-information-systems-architecture.md) → [Phase D: Technology Architecture](./phase-d-technology-architecture.md) → [Phase E–F: Migration Plan](./phase-ef-migration-plan.md).*
